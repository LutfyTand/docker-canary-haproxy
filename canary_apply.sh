...
##$DIR_DEPLOY/canary_apply.sh <<service-name>>
##############
...
function get_weight_haproxy() {
	...
	echo "[PORT $port_one] Traffic Weight: " $weight1
	echo "[PORT $port_two] Traffic Weight: " $weight2
}
...
function set_weight_docker_stable() {
	...
	if [[ $get_weight1 -eq 95 ]]; then
		echo "port $port_one"
		echo "Stabling Traffic Weight to latest deploy on port $port_one"
	...
	elif [[ $get_weight2 -eq 95 ]]; then
		echo "port $port_two"
		echo "Stabling Traffic Weight to latest deploy on port $port_two"
...
function apply_change() {
	echo "Apply deployment change to latest image"
	SERV_NAME=$1
	...
	sleep 5
	get_weight_haproxy $SERV_NAME
	echo ">>>> deployment using latest image success <<<<<"
}

echo "====== Apply deployment to latest version======"
apply_change $1
